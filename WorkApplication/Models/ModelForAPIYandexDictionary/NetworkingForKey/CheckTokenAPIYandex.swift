...
class CheckTokenAPIYandex{
    ...
    func checkTokenReceivedFromRequect(token: String?) ->  StatusToken{
        guard let token = token else {
            return .StatusTokenEmpty
        }
        print("token=",token)

        let urlStr = "https://dictionary.yandex.net/api/v1/dicservice/getLangs?key=\(token)"
    ...
    func checkConstToken(token: String?) ->  StatusToken{
        guard let token = token else {
            return .StatusTokenEmpty
        }
        print("token=",token)
